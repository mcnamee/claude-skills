# reference\templates\word\

Blank, branded `.docx` starting files - letterhead, report layout, contract
boilerplate, the meeting agenda with its example table. A new document is
**created from** one of these, inheriting its styles, headers/footers, page
setup and boilerplate; the template itself is never touched.

| | |
|---|---|
| **Read by** | the [`word`](../../../../plugins/word) plugin |
| **Setting** | `EVA_TEMPLATES_DIR` (the plugin appends `\word`) |
| **Default path** | `C:\Eva\reference\templates\word` |
...
`.pptx` templates go in [`..\powerpoint`](../powerpoint) instead: each plugin
reads only its own sub-folder, so a listing here is always just the Word
blanks.
...
`C:\Eva\documents\word`. The blanks stay blank.
