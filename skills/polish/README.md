...
APS conventions for numbers, dates, capitals, lists and links.
...
APS style is not one register, so the medium sets the shape. The skill applies one
of eight profiles:
...
The paragraph lengths are the Style Manual's own, and the profile also sets person
(`you` on a web page, `the department` in a brief), register, and format-specific
structure — review rights in a decision letter, `Summary` rather than
`Executive summary` in a report, a media contact block in a release.
...
1. **Structure** — main message into the first paragraph, headings front-loaded
   and in sentence case, action and due date out of the last line.
2. **Sentences** — 15 words on average, 25 as the hard ceiling, active voice, one
   idea each. `It was decided that` becomes `The delegate decided`.
3. **Words** — `utilise` → `use`, `prior to` → `before`, `conduct an assessment
   of` → `assess`, `in order to` → `to`. Jargon, corporate filler and noun stacks
   go. `A number of agencies` becomes the number, or gets flagged.
4. **Mechanics** — Australian spelling (`-ise`, `program`, `licence`/`license`),
   ...
   the Style Manual's three list-punctuation patterns, and descriptive link text.
5. **Inclusive and accessible language** — person-first, gender-neutral,
   `Aboriginal and Torres Strait Islander peoples`, and no idiom that fails in
   translation.
...
- **check** — a compliance report with measures (average sentence length,
  sentences over 25 words, passive constructions, undefined acronyms) and findings
  ordered by severity. Nothing is rewritten. Use it on anything already sent.
