...
- **Skill-only plugins:** a plugin may ship a skill with no MCP server (e.g. `unslop`) — it then has no `.py`, no `mcpServers`/`userConfig` block and no `__version__`, and its `plugin.json` `version` is the single source of truth. The version-sync and single-file rules above apply only to the parts a skill-only plugin actually has.
