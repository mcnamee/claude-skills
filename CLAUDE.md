...
- **Shared flag vocabulary:** `--docs-dir` (the source-documents folder a server is confined to), `--output-dir` (generated files), `--templates-dir` (blank templates a new file is created from — a READ-ONLY root: readable, but every save into it is refused), `--kb-dir` (optional Markdown mirror for the RAG knowledge base), `--base-url`/`--ca-cert`/`--insecure`/`--timeout`/`--max-body` (HTTP servers), `--check` (validate config/connectivity and exit), `--version`. Reuse these names for new servers/settings; do not invent synonyms (no `--folder`, `--input-dir`, `--document-root`).
- **Context folder:** `context/exemplars` (finished documents showing what good output looks like) and `context/templates` (blank `.docx`/`.pptx` files new documents are created from) are reference material, not code — no `plugin.json`, no version. Each carries a `README.md`, not a `CLAUDE.md`: they are consumed on the user's endpoint, not by an agent working in this repo. The document files themselves are gitignored (`context/.gitignore`) because they are usually the user's real corporate documents and this repo is public; keep it that way unless asked. A server that needs templates points at `context/templates` with `--templates-dir` (the `word` server does).
